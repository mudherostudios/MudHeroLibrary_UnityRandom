...
namespace URandom
{
	public static class RandomCube
	{
		...
		public static Vector3 Surface(ref NPack.MersenneTwister _rand, MudHeroRandom.Normalization n, float t)
		{
			Vector3 pos = new Vector3();
			switch (n) {
			case MudHeroRandom.Normalization.STDNORMAL:
				pos = GetPointOnCubeSurface(
					(float) NormalDistribution.Normalize(_rand.NextSingle(true), t),
					(float) NormalDistribution.Normalize(_rand.NextSingle(true), t),
					_rand.Next(5));
			break;
			case MudHeroRandom.Normalization.POWERLAW:
				pos = GetPointOnCubeSurface(
		...
		public static Vector3 Volume(ref NPack.MersenneTwister _rand, MudHeroRandom.Normalization n, float t)
		{
			float x, y, z;
			x = y = z = 0;
			switch (n) {
